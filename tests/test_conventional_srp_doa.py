...
from visualization.grids import plot_azimuth_elevation_grid, plot_uniform_cartesian_grid
...
def test_doa_3d():
    ...
    axs[1].set_title("High-rt60 (0.9s) without interference")
    plot_azimuth_elevation_grid(
        candidate_grid, output_path="tests/temp/srp_doa_az_el.png", srp_map=srp_map_reverb,
    ...
    axs[2].set_title("Low-reverb with interference (SNR=10dB)")
    plot_azimuth_elevation_grid(
        candidate_grid, output_path="tests/temp/srp_doa_az_el.png", srp_map=srp_map,
        source_positions=source_position_norm[np.newaxis, :],
        interferer_positions=interferer_position_norm[np.newaxis, :], ax=axs[2]
    )

    plt.savefig("tests/temp/srp_doa_az_el.png")
...
def _simulate_3d(rt60=0.3, interferer=False, interferer_snr=10, scenario=None):
    ...
    if interferer:
        ...
        scaling_factor = np.sqrt(
            np.sum(room.mic_array.signals[:, :fs] ** 2)
            / np.sum(room.sources[0].signal ** 2)
        )
        scaling_factor = 10 ** (-interferer_snr / 20) / scaling_factor
        
        signals += scaling_factor * room.mic_array.signals[:, :fs]
    return (
...
def generate_random_scenario(mode="2d", interferer=False):
    ...
    if mode == "2d":
        ...
        if interferer:
            ...
            interferer_position = interferer_position_norm + np.array([5, 5])
        ...
        mic_positions = mic_positions_norm + np.array([5, 5, 5])
        # Place the source 1 meter away from the microphone array at a random angle
        ...
        source_position = source_position_norm + np.array([5, 5, 5])
        ...
        if interferer:
            ...
            interferer_position = interferer_position_norm + np.array([5, 5, 5])
